...
def makeGeometry(weight_frac_old, weight_frac_new, dt, T = 280, avgdensity = False):
    ...
    if avgdensity:
        cell_fuel_old  = openmc.Cell(name='fuel', fill=fuel, region=fuel_region_old)
        cell_fuel_new  = openmc.Cell(name='fuel', fill=fuel, region=fuel_region_new)
    else:
...
def keff_T(weight_frac_old, weight_frac_new, dt,T = 280, avgdensity = False):
    model = makeModel(weight_frac_old, weight_frac_new, dt, T, avgdensity)
